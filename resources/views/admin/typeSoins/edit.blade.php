...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.type-soin.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.type-soins.update", [$typeSoin->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="name">{{ trans('cruds.type-soin.fields.name') }}</label>
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $typeSoin->name) }}" required>
                @if($errors->has('name'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('prix') ? 'is-invalid' : '' }}" type="number" name="prix" id="prix" value="{{ old('prix', $typeSoin->prix) }}" step="0.01" required>
                @if($errors->has('prix'))
                    <div class="invalid-feedback">
                        {{ $errors->first('prix') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('caisse') ? 'is-invalid' : '' }}" name="caisse_id" id="caisse_id" required>
                    @foreach($caisses as $id => $caisse)
                        <option value="{{ $id }}" {{ ($typeSoin->caisse ? $typeSoin->caisse->id : old('caisse_id')) == $id ? 'selected' : '' }}>{{ $caisse }}</option>
                    @endforeach
                </select>
