...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.operationCash.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.operation-cashes.update", [$operationCash->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('caisse') ? 'is-invalid' : '' }}" name="caisse_id" id="caisse_id" required>
                    @foreach($caisses as $id => $caisse)
                        <option value="{{ $id }}" {{ ($operationCash->caisse ? $operationCash->caisse->id : old('caisse_id')) == $id ? 'selected' : '' }}>{{ $caisse }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('medecin') ? 'is-invalid' : '' }}" name="medecin_id" id="medecin_id" required>
                    @foreach($medecins as $id => $medecin)
                        <option value="{{ $id }}" {{ ($operationCash->medecin ? $operationCash->medecin->id : old('medecin_id')) == $id ? 'selected' : '' }}>{{ $medecin }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('montant') ? 'is-invalid' : '' }}" type="number" name="montant" id="montant" value="{{ old('montant', $operationCash->montant) }}" step="0.01" required>
