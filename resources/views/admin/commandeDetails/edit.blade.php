...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.commandeDetail.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.commande-details.update", [$commandeDetail->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('commande') ? 'is-invalid' : '' }}" name="commande_id" id="commande_id" required>
                    @foreach($commandes as $id => $commande)
                        <option value="{{ $id }}" {{ ($commandeDetail->commande ? $commandeDetail->commande->id : old('commande_id')) == $id ? 'selected' : '' }}>{{ $commande }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('article') ? 'is-invalid' : '' }}" name="article_id" id="article_id" required>
                    @foreach($articles as $id => $article)
                        <option value="{{ $id }}" {{ ($commandeDetail->article ? $commandeDetail->article->id : old('article_id')) == $id ? 'selected' : '' }}>{{ $article }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('quantity') ? 'is-invalid' : '' }}" type="number" name="quantity" id="quantity" value="{{ old('quantity', $commandeDetail->quantity) }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('prix_unitaire') ? 'is-invalid' : '' }}" type="number" name="prix_unitaire" id="prix_unitaire" value="{{ old('prix_unitaire', $commandeDetail->prix_unitaire) }}" step="1">
