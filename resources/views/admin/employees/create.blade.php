    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.employees.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('matricule') ? 'is-invalid' : '' }}" type="number" name="matricule" id="matricule" value="{{ old('matricule', '') }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('nni') ? 'is-invalid' : '' }}" type="number" name="nni" id="nni" value="{{ old('nni', '') }}" step="1" required>
                @if($errors->has('nni'))
                    <div class="invalid-feedback">
                        {{ $errors->first('nni') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('emploi') ? 'is-invalid' : '' }}" name="emploi_id" id="emploi_id" required>
                    @foreach($emplois as $id => $entry)
                        <option value="{{ $id }}" {{ old('emploi_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('recruitement_date') ? 'is-invalid' : '' }}" type="text" name="recruitement_date" id="recruitement_date" value="{{ old('recruitement_date') }}" required>
