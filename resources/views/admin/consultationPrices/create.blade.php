...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.consultationPrice.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.consultation-prices.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('tarif') ? 'is-invalid' : '' }}" type="number" name="tarif" id="tarif" value="{{ old('tarif', '') }}" step="1" required>
                @if($errors->has('tarif'))
                    <div class="invalid-feedback">
                        {{ $errors->first('tarif') }}
                    </div>
