...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.charge.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.charges.update", [$charge->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label for="amount">{{ trans('cruds.charge.fields.amount') }}</label>
                <input class="form-control {{ $errors->has('amount') ? 'is-invalid' : '' }}" type="number" name="amount" id="amount" value="{{ old('amount', $charge->amount) }}" step="0.01">
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('dt_charge') ? 'is-invalid' : '' }}" type="text" name="dt_charge" id="dt_charge" value="{{ old('dt_charge', $charge->dt_charge) }}" required>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('description') ? 'is-invalid' : '' }}" name="description" id="description">{{ old('description', $charge->description) }}</textarea>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('motif') ? 'is-invalid' : '' }}" name="motif_id" id="motif_id" required>
                    @foreach($motifs as $id => $entry)
                        <option value="{{ $id }}" {{ (old('motif_id') ? old('motif_id') : $charge->motif->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('motif'))
                    <div class="invalid-feedback">
